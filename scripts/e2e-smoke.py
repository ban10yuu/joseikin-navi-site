...
with sync_playwright() as playwright:
    ...
    for visual_width in [320, 768, 1440]:
        ...
        check(affiliate_image.count() == 2, f'トップ ({visual_width}px): 公式PR画像が2件ではありません')
    ...
    check(desktop_pr.count() == 2, 'トップPC: 検索フォーム右のPR枠が2件ではありません')
    if desktop_pr.count() == 2:
        ...
        desktop_second_pr_box = desktop_pr.nth(1).bounding_box()
        ...
        check(desktop_second_pr_box['y'] >= desktop_pr_box['y'] + desktop_pr_box['height'], 'トップPC: 2件のPR枠が重なっています')
    ...
    check(hero_pr.count() == 2, 'トップに事業者向けPRが2件表示されません')
    if hero_pr.count() == 2:
    ...
    check(home.locator('[data-analytics-event="affiliate_impression"][data-placement="home-business-entry"]').count() == 0, '削除した事業者向け入口のPR枠が残っています')
    check(home.locator('[data-analytics-event="affiliate_impression"]').count() == 2, 'トップのPR枠が2件ではありません')
    ...
    check(ended.locator('[data-analytics-event="affiliate_impression"]').count() == 2, '終了制度の詳細ページにPR枠が2件表示されません')
    ...
    check(ended_business.locator('[data-analytics-event="affiliate_impression"]').count() == 2, '終了済み創業制度の詳細ページにPR枠が2件表示されません')
    ...
    check(sensitive_impression.count() == 2, '個人向け・医療系の制度詳細にPR枠が2件表示されません')
    check(sensitive_impression.nth(0).get_attribute('data-audience') in ('individual', 'family'), '個人向け制度の代表ページを検証できていません')
    check(sensitive_impression.nth(0).get_attribute('data-purpose') == 'medical', '医療系制度の代表ページを検証できていません')
    ...
    check(affiliate_desktop_pr.count() == 2, '制度詳細PC: 右レールのPR枠が2件ではありません')
    if affiliate_desktop_pr.count() == 2:
    ...
    check(affiliate_impression.count() == 2, '関連する事業者向け制度にPR枠が2件表示されません')
    check(affiliate_link.count() == 2, 'PR枠のリンクが2件ではありません')
    ...
    check(business_guide.locator('[data-analytics-event="affiliate_impression"][data-page-type="businessGuide"]').count() == 2, '事業者向け申請ガイドに関連PRが2件表示されません')
