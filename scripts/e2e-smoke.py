...
with sync_playwright() as playwright:
    ...
    if hero_pr.count() == 1:
        ...
        check(hero_pr.locator('.affiliate-banner-visual').is_visible(), 'トップPRにビジュアル要素がありません')
    ...
    check(home.locator('[data-analytics-event="affiliate_impression"]').count() == 1, 'トップのPR枠が1件ではありません')
    ...
    check(affiliate_impression.locator('.affiliate-banner-visual').is_visible(), '制度詳細PRにビジュアル要素がありません')
    check('サービスを見る' in (affiliate_link.inner_text() or ''), 'PRリンクのボタン文言がサービスを見るではありません')
