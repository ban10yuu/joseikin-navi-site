...
const issueCodes = new Set([
  'APPLICATION_PERIOD_NAV_CONTAMINATION',
  'PUBLIC_NAV_CONTAMINATION',
]);
const publicNavigationPattern = /現在位置\s+ホーム|この記事と同じ分類の記事|共通メニューなどをスキップして本文へ/;
...
for (const issue of report.issues.filter((item) => issueCodes.has(item.code))) {
  const codes = targetIssues.get(issue.slug) ?? new Set();
  codes.add(issue.code);
  targetIssues.set(issue.slug, codes);
}
const targetSlugs = [...targetIssues.keys()];
