...
const cities = [
  { name: '川越市', pref: '埼玉県', prefix: 'kawagoe-', desc: '川越市は「小江戸」の愛称で親しまれ、蔵造りの町並みや時の鐘が有名な観光都市です。さつまいもを使った菓子や川越まつりなど江戸情緒あふれるまちです。' },
  { name: '所沢市', pref: '埼玉県', prefix: 'tokorozawa-', desc: '所沢市は日本初の飛行場が開設された航空発祥の地で、所沢航空記念公園が市民の憩いの場です。西武ライオンズの本拠地としても知られています。' },
  { name: '川口市', pref: '埼玉県', prefix: 'kawaguchi-', desc: '川口市は埼玉県南部に位置し、東京に隣接する人口60万人超の大都市です。鋳物産業の歴史があり、近年は住みやすさで注目される住宅都市に発展しています。' },
  { name: '越谷市', pref: '埼玉県', prefix: 'koshigaya-', desc: '越谷市は埼玉県東部に位置し、日本最大級のショッピングモール「イオンレイクタウン」があるまちです。越谷だるまや久伊豆神社など歴史文化も残っています。' },
  { name: '草加市', pref: '埼玉県', prefix: 'soka-', desc: '草加市は埼玉県南東部に位置し、草加せんべいの発祥地として全国的に有名です。日光街道の宿場町として栄えた歴史があり、草加松原は国の名勝に指定されています。' },
  { name: '春日部市', pref: '埼玉県', prefix: 'kasukabe-', desc: '春日部市は埼玉県東部の都市で、アニメ「クレヨンしんちゃん」の舞台として知られています。首都圏外郭放水路（地下神殿）は巨大な治水施設として人気の見学スポットです。' },
];
...
function genGrants(c: typeof cities[0]): Grant[] {
  ...
  const cats: Array<{cat: Grant['category'], slug: string, title: string, amount: string, amountNum: number, elig: string, period: string, descr: string, s1: string, s2: string, s3: string, tags: string[]}> = [
    { cat: 'childcare', slug: `${c.prefix}child-medical-aid`, title: `${c.name} こども医療費助成`, amount: '自己負担なし（全額助成）', amountNum: 0, elig: `${c.name}に住所を有する18歳までの子どもの保護者`, period: '通年', descr: `${c.name}が実施するこども医療費助成制度です。18歳年度末まで医療費の自己負担を全額助成します。`, tags: [c.name, 'こども医療費', pref],
      ...
      s3: `<p>通院・入院ともに<strong>自己負担なし</strong>。保険適用分の自己負担額が全額助成されます。</p><p><span class="marker">県内医療機関は受給資格証提示で窓口無料</span>。県外は後日申請で償還。</p><div class="note-box">${pref}は子ども医療費助成が充実しています。入院時の食事代は助成対象外です。</div>` },
...
export const cityGrantsBatch99: Grant[] = cities.flatMap(c => genGrants(c));
