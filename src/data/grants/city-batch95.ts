...
const cities = [
  { name: '旭川市', pref: '北海道', prefix: 'asahikawa-', desc: '旭川市は北海道第2の都市で、旭山動物園の行動展示で全国的に有名です。旭川ラーメンや旭川家具など、食と工芸の文化が根付くまちです。' },
  { name: '函館市', pref: '北海道', prefix: 'hakodate-', desc: '函館市は北海道南部の港町で、函館山からの夜景は世界三大夜景の一つに数えられます。五稜郭や赤レンガ倉庫群など歴史的観光スポットが豊富です。' },
  { name: '釧路市', pref: '北海道', prefix: 'kushiro-', desc: '釧路市は北海道東部の中心都市で、釧路湿原国立公園のタンチョウヅルで知られています。漁業が盛んで、炉端焼き発祥の地としても有名です。' },
  { name: '帯広市', pref: '北海道', prefix: 'obihiro-', desc: '帯広市は十勝平野の中心都市で、広大な農業地帯と豚丼・スイーツのまちとして知られています。ばんえい競馬は世界唯一の輓曳競馬として人気を集めています。' },
  { name: '小樽市', pref: '北海道', prefix: 'otaru-', desc: '小樽市は北海道西部の港町で、小樽運河やガラス工芸が有名な観光都市です。かつてのニシン漁で栄えた歴史があり、寿司屋通りなど食の魅力も豊富です。' },
  { name: '苫小牧市', pref: '北海道', prefix: 'tomakomai-', desc: '苫小牧市は北海道の太平洋岸に位置する工業都市で、製紙業と港湾を基盤に発展しました。ウトナイ湖はラムサール条約登録湿地として野鳥の楽園です。' },
];
...
function genGrants(c: typeof cities[0]): Grant[] {
  ...
  const cats: Array<{cat: Grant['category'], slug: string, title: string, amount: string, amountNum: number, elig: string, period: string, descr: string, s1: string, s2: string, s3: string, tags: string[]}> = [
    { cat: 'childcare', slug: `${c.prefix}child-medical-aid`, title: `${c.name} こども医療費助成`, amount: '自己負担なし（全額助成）', amountNum: 0, elig: `${c.name}に住所を有する18歳までの子どもの保護者`, period: '通年', descr: `${c.name}が実施するこども医療費助成制度です。18歳年度末まで医療費の自己負担を全額助成します。`, tags: [c.name, 'こども医療費', pref],
      ...
      s2: `<p>${c.name}に住民登録のある18歳年度末までの子どもが対象です。</p><div class="summary-box" data-title="申請に必要なもの"><p>子どもの健康保険証</p><p>保護者の本人確認書類</p><p>振込先口座情報</p></div><p>${c.name}子育て支援課窓口で<span class="marker">受給資格証の交付申請</span>を行ってください。道外医療機関の受診は償還払いとなります。</p>`,
      s3: `<p>通院・入院ともに<strong>自己負担なし</strong>。保険適用分の自己負担額が全額助成されます。</p><p><span class="marker">道内医療機関は受給資格証提示で窓口無料</span>。道外は後日申請で償還。</p><div class="note-box">${pref}は子ども医療費助成が充実しています。入院時の食事代は助成対象外です。</div>` },
...
export const cityGrantsBatch95: Grant[] = cities.flatMap(c => genGrants(c));
