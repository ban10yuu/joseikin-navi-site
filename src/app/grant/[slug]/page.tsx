...
import { CATEGORY_LABELS, PREFECTURES, SUPPORT_TYPE_LABELS, type Section } from '@/lib/types';
    ...
    <>
      ...
      <div className={`grant-detail-page${showMonetizationRail ? ' has-affiliate' : ''}`}>
        ...
        <div className="grant-detail-layout">
          ...
          {related.length > 0 && <section className="grant-related" aria-labelledby="related-grants-title"><div className="home-section-heading"><p>対象・目的・地域が近い制度</p><h2 id="related-grants-title">関連する制度</h2></div><div className="grant-related-grid">{related.map((item, index) => <GrantCard key={item.slug} grant={item} pageType="grant" placement="related_grants" position={index + 1} />)}</div></section>}
