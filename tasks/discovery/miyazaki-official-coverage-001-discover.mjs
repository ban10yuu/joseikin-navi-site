...
const batch = 'miyazaki-official-coverage-001';
const keywords = ['補助金', '助成金', '給付金', '支援金', '手当', '医療費助成', '就学援助', '奨学金', '貸付', '減免', '移住', '空き家', '耐震', '創業'];
...
const orgs = [
  {
    ...
    sitemaps: [],
    searches: keywords.map((q) => `https://www.pref.miyazaki.lg.jp/search.html?keyword=${encodeURIComponent(q)}`),
    seeds: ['/', '/kurashi/', '/kenko/', '/shigoto/', '/kense/', '/kurashi/kosodate/', '/shigoto/sangyo/', '/shigoto/nogyo/', '/kurashi/iju/', '/kenko/fukushi/'],
    keep: /\/(?:kurashi|kenko|shigoto|kense|contents|documents)[^"'<> ]*(?:\.html|\.pdf|\/)(?:$|\?)/i,
    crawl: /\/(?:kurashi|kenko|shigoto|kense)(?:\/|\.html|$)/i,
    limit: 550,
  },
  {
    org: '宮崎市',
    base: 'https://www.city.miyazaki.miyazaki.jp',
    sitemaps: [],
    searches: keywords.map((q) => `https://www.city.miyazaki.miyazaki.jp/search?keyword=${encodeURIComponent(q)}`),
    seeds: ['/', '/life/', '/health/', '/education/', '/business/', '/city/', '/life/house/', '/health/welfare/', '/education/childbirth/', '/business/loan/', '/business/architecture/'],
    keep: /\/(?:life|health|education|business|city)[^"'<> ]*(?:\.html|\.pdf|\/)(?:$|\?)/i,
    crawl: /\/(?:life|health|education|business|city)(?:\/|\.html|$)/i,
    limit: 650,
  },
  {
    org: '都城市',
    base: 'https://www.city.miyakonojo.miyazaki.jp',
    sitemaps: [],
    searches: keywords.map((q) => `https://www.city.miyakonojo.miyazaki.jp/search.html?keyword=${encodeURIComponent(q)}`),
    seeds: ['/', '/site/iju/', '/site/kosodate/', '/soshiki/', '/life/', '/site/', '/soshiki/20/', '/soshiki/66/', '/soshiki/3/', '/soshiki/40/'],
    keep: /\/(?:site|soshiki|life|uploaded)[^"'<> ]*(?:\.html|\.pdf|\/)(?:$|\?)/i,
    crawl: /\/(?:site|soshiki|life)(?:\/|\.html|$)/i,
    limit: 650,
  },
  {
    org: '延岡市',
    base: 'https://www.city.nobeoka.miyazaki.jp',
    sitemaps: [],
    searches: keywords.map((q) => `https://www.city.nobeoka.miyazaki.jp/search.html?keyword=${encodeURIComponent(q)}`),
    seeds: ['/', '/site/miryoku/', '/site/kosodate/', '/soshiki/', '/life/', '/soshiki/24/', '/soshiki/32/', '/soshiki/33/', '/soshiki/40/', '/soshiki/51/'],
    keep: /\/(?:site|soshiki|life|uploaded)[^"'<> ]*(?:\.html|\.pdf|\/)(?:$|\?)/i,
    crawl: /\/(?:site|soshiki|life)(?:\/|\.html|$)/i,
    limit: 650,
  },
  {
    org: '日南市',
    base: 'https://www.city.nichinan.lg.jp',
    sitemaps: ['https://www.city.nichinan.lg.jp/sitemap.xml'],
    searches: keywords.map((q) => `https://www.city.nichinan.lg.jp/search.html?keyword=${encodeURIComponent(q)}`),
    seeds: ['/', '/kurashi_tetsuzuki/', '/kenko_iryo_fukushi/', '/kosodate_kyoiku/', '/shigoto_sangyo/', '/soshikikarasagasu/', '/kurashi_tetsuzuki/kekkonshien/', '/kurashi_tetsuzuki/sumai/', '/shigoto_sangyo/shokogyo/'],
    keep: /\/(?:kurashi_tetsuzuki|kenko_iryo_fukushi|kosodate_kyoiku|shigoto_sangyo|soshikikarasagasu|material)[^"'<> ]*(?:\.html|\.pdf|\/)(?:$|\?)/i,
    crawl: /\/(?:kurashi_tetsuzuki|kenko_iryo_fukushi|kosodate_kyoiku|shigoto_sangyo|soshikikarasagasu)(?:\/|\.html|$)/i,
    limit: 650,
  },
];
...
const target = /(対象|要件|県内|市内|住民|世帯|児童|妊婦|高齢者|障害|障がい|事業者|所有者|保護者|申請者|所得|市民|若者|中小企業|移住者|農業者|漁業者)/;
const period = /(申請|受付|募集|締切|期限|提出|交付|請求|公開日|更新日|令和|20\d{2}年)/;
const bad = /(議会|入札|発注|プロポーザル|職員採用|広報|統計|計画|報告|実績|審議|選挙|監査|個人情報|会議|パブリックコメント|予算|決算|求人|公告|工事|委託|指定管理|アンケート|審査結果|採択事業者|アクセス|施設予約|市長の日程|知事の日程|ごみ分別|イベント|講座|セミナー)/;
...
function scoreCandidate(title, text, url) {
  ...
  if (/\/(?:soshiki|site|business|health|education|kurashi_tetsuzuki|shigoto_sangyo)\/|20\d{2}|\/\d+\.html/.test(url)) score += 2;
...
for (const org of orgs) {
  ...
  const pages = await mapLimit([...urls].slice(0, org.limit), 10, async (url) => {
    ...
    if (/\/(?:category|life|site|soshiki)\/?$|\/search/.test(page.url) && !/(補助金|助成金|給付金|支援|手当|医療|子育て|福祉|移住|空き家|住宅|教育|就学|奨学)/.test(title)) return null;
