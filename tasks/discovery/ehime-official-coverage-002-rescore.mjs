...
const batch = 'ehime-official-coverage-002';
const outFile = `tasks/discovery/${batch}-rescored-candidates.json`;

const municipalities = [
  { org: '宇和島市', base: 'https://www.city.uwajima.ehime.jp', seeds: ['/', '/soshiki/', '/site/', '/kurashi/', '/kosodate/', '/business/', '/life/'], mode: 'cms', max: 850 },
  { org: '八幡浜市', base: 'https://www.city.yawatahama.ehime.jp', seeds: ['/', '/soshiki/', '/site/', '/kurashi/', '/kosodate/', '/business/', '/life/'], mode: 'cms', max: 800 },
  { org: '新居浜市', base: 'https://www.city.niihama.lg.jp', seeds: ['/', '/soshiki/', '/site/', '/kurashi/', '/kosodate/', '/business/', '/life/'], mode: 'cms', max: 850 },
  { org: '西条市', base: 'https://www.city.saijo.ehime.jp', seeds: ['/', '/soshiki/', '/site/', '/kurashi/', '/kosodate/', '/business/', '/life/'], mode: 'cms', max: 850 },
  { org: '大洲市', base: 'https://www.city.ozu.ehime.jp', seeds: ['/', '/soshiki/', '/site/', '/kurashi/', '/kosodate/', '/business/', '/life/'], mode: 'cms', max: 800 },
];
...
function usefulUrl(orgBase, url) {
  ...
  return pathHint.test(decodeURIComponent(url)) || /\/(soshiki|site|page|docs|kurashi|life|gyosei|gyousei|contents|article|detail|news|category|living|health|welfare|child|business)\//.test(url) || /\.php\?/.test(url);
}
...
async function collectUrls(m) {
  ...
  while (queue.length && urls.size < m.max) {
    ...
    for (const link of linksFrom(url, page.html)) {
      ...
      if ((/\/(life|soshiki|site|kurashi|gyosei|gyousei|contents|category|article|detail|news)\//.test(link) || pathHint.test(decodeURIComponent(link))) && queue.length < 1100) queue.push(link);
    }
  }
  return [...urls].slice(0, m.max);
}
...
function scoreCandidate(title, text, url) {
  const hay = `${title} ${text.slice(0, 3000)} ${decodeURIComponent(url)}`;
  let score = 0;
  ...
  if (pathHint.test(decodeURIComponent(url))) score += 3;
