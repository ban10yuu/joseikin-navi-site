...
const { selections } = JSON.parse(fs.readFileSync('tasks/discovery/nagasaki-official-coverage-002-selections.json','utf8'));
const outBase='tasks/discovery/nagasaki-official-coverage-002';
...
const cachedPages = fs.existsSync(browserLinksPath)
  ...
  : [];
const cachedByUrl = new Map(cachedPages.map((page) => [
  page.url,
  {
    status: page.status || 200,
    url: page.url,
    text: `${page.title} ${page.evidence}`.replace(/\s+/g, ' ').trim(),
  },
]));
...
fs.writeFileSync(outBase+'-ts-snippet.ts',`const nagasakiOfficial002Grants: Array<{
  slug: string;
  title: string;
  organization: string;
  maxAmount: string;
