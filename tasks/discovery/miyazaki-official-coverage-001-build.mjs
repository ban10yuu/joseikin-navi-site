...
const { selections } = JSON.parse(fs.readFileSync('tasks/discovery/miyazaki-official-coverage-001-selections.json', 'utf8'));
const outBase = 'tasks/discovery/miyazaki-official-coverage-001';
...
function category(title) {
  if (/医療|不妊|不育|妊婦|未熟児|ドナー|税|減免/.test(title)) return 'medical';
  if (/児童|子ども|こども|子育て|母子|父子|ひとり親|養育/.test(title)) return 'childcare';
  if (/住宅|空き家|空家|住居|定住|移住|リフォーム|耐震|ブロック塀|浄化槽/.test(title)) return 'housing';
  if (/奨学|就学|教育|学校/.test(title)) return 'education';
  if (/障害|障がい|福祉|介護|高齢|エアコン/.test(title)) return 'nursing';
  if (/創業|事業|中小|商業|店舗|雇用|就労|産業|融資|農業|職場/.test(title)) return 'employment';
  return 'living';
}
...
function related(title, cat) {
  ...
  if (/児童|子ども|こども|子育て|妊婦|母子|父子|ひとり親|養育/.test(title)) add('childcare');
  if (/医療|健康|不妊|不育|ドナー|未熟児|税|減免/.test(title)) add('medical');
  if (/住宅|空き家|空家|住居|定住|移住|リフォーム|耐震|ブロック塀|浄化槽/.test(title)) add('housing');
  if (/奨学|就学|教育|学校/.test(title)) add('education');
  if (/商業|事業|雇用|就労|創業|企業|店舗|融資|農業|職場/.test(title)) add('employment');
  if (/障害|障がい|介護|福祉|高齢|エアコン/.test(title)) add('nursing');
  add('living');
...
function maxAmount(title, text) {
  ...
  if (/給付金|支援金|奨励金/.test(title)) return { text: '公式ページ記載の給付額・支援額に従う', num: 0 };
...
function focusText(text, title) {
  ...
  if (start >= 0) return text.slice(start, start + 3000);
...
function application(text, title) {
  ...
  let phrase = body.match(/(?:申請|募集|受付|締切|期限|交付|支給|請求|提出)[^。．]{0,110}/)?.[0] || body.slice(0, 120);
  phrase = phrase.split(/[｜|/]/)[0].trim();
  if (/本文へ|Foreign|フッターメニュー|緊急情報|メインメニュー|ページの先頭|ヘッダー/.test(phrase)) phrase = '公式ページで申請・受付状況を確認';
...
async function fetchPage(url) {
  try {
    const { stdout } = await execFileAsync('curl', [
      ...
      url,
    ], { timeout: 22000, maxBuffer: 1024 * 1024 * 4 });
...
for (let i = 0; i < selections.length; i++) {
  ...
  adopted.push({
    slug: `miyazaki-official-001-${String(i + 1).padStart(3, '0')}-2026`,
    title: `${org} ${name}`,
    organization: org,
...
fs.writeFileSync(`${outBase}-ts-snippet.ts`, `const miyazakiOfficial001Grants: Array<{
  slug: string;
  title: string;
  organization: string;
  maxAmount: string;
