...
const orgs = [
  {
    ...
    sitemaps: ['https://www.city.tamana.lg.jp/sitemap.xml'],
    seeds: ['/q/list/1.html', '/q/list/2.html', '/q/list/3.html', '/q/list/4.html', '/q/list/5.html'],
    keep: /\/q\/aview\/\d+\/\d+\.html/i,
  },
  {
    org: '山鹿市',
    base: 'https://www.city.yamaga.kumamoto.jp',
    sitemaps: ['https://www.city.yamaga.kumamoto.jp/sitemap.xml'],
    seeds: ['/kosodate/', '/kenko/', '/kurashi/', '/shigoto/', '/ijyu/'],
    keep: /\/(?:kosodate|kenko|kurashi|shigoto|ijyu|fukushi)\/kiji\d+\/index\.html/i,
  },
  {
    org: '菊池市',
    base: 'https://www.city.kikuchi.lg.jp',
    sitemaps: ['https://www.city.kikuchi.lg.jp/sitemap.xml'],
    seeds: ['/article/list/900.html', '/category/list/1001.html', '/category/list/1002.html', '/category/list/1003.html', '/category/list/1004.html'],
    keep: /\/article\/view\/\d+\/\d+\.html/i,
  },
  {
    org: '宇土市',
    base: 'https://www.city.uto.lg.jp',
    sitemaps: [],
    seeds: ['/category/list/1001.html', '/category/list/1012.html', '/category/list/1054.html', '/category/list/1070.html', '/category/list/1090.html', '/category/list/1105.html', '/article/keyword/22145.html', '/article/keyword/106.html', '/article/keyword/177.html'],
    keep: /\/article\/view\/\d+\/\d+\.html/i,
  },
  {
    org: '上天草市',
    base: 'https://www.city.kamiamakusa.kumamoto.jp',
    sitemaps: ['https://www.city.kamiamakusa.kumamoto.jp/sitemap.xml'],
    seeds: ['/q/list/400.html', '/q/list/500.html', '/q/list/600.html', '/q/list/700.html', '/q/list/800.html'],
    keep: /\/q\/aview\/\d+\/\d+\.html/i,
  },
];

const strong = /(補助金|助成金|給付金|支援金|奨学金|手当|貸付|無償化|医療費助成|就学援助|減免|補助|助成|給付|支援制度|交付金|奨励金|融資|優遇制度|無料化|助成制度|補助制度)/;
const amount = /(上限|最大|限度|月額|年額|円|万円|補助率|支給額|助成額|補助額|自己負担|無料|全額|半額|免除|減免|定額)/;
const target = /(対象|要件|市内|住民|世帯|児童|妊婦|高齢者|障害|障がい|事業者|所有者|保護者|申請者|所得|市民|若者|中小企業|移住者)/;
const period = /(申請|受付|募集|締切|期限|提出|交付|請求|公開日|更新日|令和|20\d{2}年)/;
const bad = /(議会|入札|発注|プロポーザル|職員採用|広報|統計|計画|報告|実績|審議|選挙|監査|個人情報|会議|パブリックコメント|予算|決算|求人|公告|工事|委託|指定管理|アンケート|審査結果|採択事業者|アクセス|施設予約)/;
...
function scoreCandidate(title, text, url) {
  const hay = `${title} ${text.slice(0, 3200)} ${url}`;
  let score = 0;
  ...
  score += Math.min(strongHits, 7) * 2;
  if (/aview|article\/view|kiji/.test(url)) score += 2;
  if (bad.test(hay)) score -= 12;
  if (/一覧|カテゴリ|トップページ|サイトマップ|検索結果|予約/.test(title)) score -= 8;
  return score;
}
...
for (const org of orgs) {
  ...
  for (const sitemap of org.sitemaps) {
    const page = await fetchPage(sitemap);
    if (page.status !== 200) continue;
    for (const m of page.html.matchAll(/<loc>([^<]+)<\/loc>/gi)) {
      const url = m[1].replaceAll('&amp;', '&').trim();
      if (url.startsWith(org.base) && org.keep.test(url)) urls.add(url);
    }
  }
  const seedPages = await mapLimit(org.seeds.map((seed) => absolute(org.base, seed)), 5, (url) => fetchPage(url));
  for (const page of seedPages) {
    if (page.status !== 200) continue;
    for (const link of linksFrom(page.html, page.url, org)) urls.add(link);
  }

  const pages = await mapLimit([...urls].slice(0, 120), 10, async (url) => {
    const page = await fetchPage(url);
    if (page.status !== 200) return null;
    const title = titleOf(page.html, url);
    const score = scoreCandidate(title, page.text, page.url);
    if (score < 16) return null;
    return { org: org.org, title, url: page.url, status: page.status, score, evidence: page.text.slice(0, 280) };
  });
...
fs.writeFileSync(`tasks/discovery/${batch}-focused-candidates.json`, JSON.stringify({
...
fs.writeFileSync(`tasks/discovery/${batch}-focused-candidates.tsv`, [
