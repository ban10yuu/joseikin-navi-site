...
const batch = 'oita-official-coverage-003';
const keywords = ['補助金', '助成金', '給付金', '支援金', '手当', '医療費助成', '就学援助', '奨学金', '貸付', '減免', '移住', '空き家', '耐震', '無償化'];

const orgs = [
  {
    org: '宇佐市',
    base: 'https://www.city.usa.oita.jp',
    seeds: ['/', '/sitemap.xml', '/sougo/kurashi_tetsuzuki/index.html', '/sougo/iryo_kenko_fukushi/index.html', '/sougo/kosodate_kyoiku/index.html', '/sougo/jigyosha/index.html', '/sougo/soshiki/index.html', '/sougo/2_1/index.html'],
    searches: keywords.map((q) => `https://www.city.usa.oita.jp/search.html?q=${encodeURIComponent(q)}`),
    keep: /\/(?:sougo|tourist|usacoco)\/[^"'<> ]*(?:\.html|\/)(?:$|\?)/i,
    crawl: /\/(?:sougo|tourist|usacoco|sitemap)(?:\/|\.html|\.xml|$)/i,
    limit: 900,
  },
  {
    org: '豊後大野市',
    base: 'https://www.bungo-ohno.jp',
    seeds: ['/', '/sitemap.html', '/categories/bunyabetsu/', '/categories/bunyabetsu/zei_nenkin_kokuho/', '/categories/bunyabetsu/hoken_iryou_kaigo_fukushi/', '/categories/bunyabetsu/anzen_kankyou_jyutaku_suidou/', '/categories/bunyabetsu/syoukou_roudou/', '/categories/bunyabetsu/nou_rin/', '/categories/bunyabetsu/kyouiku_bunka_sports_jinken/'],
    searches: keywords.map((q) => `https://www.bungo-ohno.jp/search?keyword=${encodeURIComponent(q)}`),
  ...
  {
    org: '由布市',
    base: 'https://www.city.yufu.oita.jp',
    seeds: ['/', '/kurashi', '/kurashi/nenkinhoken', '/kurashi/kenkouiryou', '/kurashi/fukusikaigo', '/kurashi/kyoikugakko', '/kurashi/sumai_akiya', '/kurashi/syoukougyo', '/kurashi/nouseinougyouiinnkai', '/kurashi/bousai'],
    searches: keywords.map((q) => `https://www.city.yufu.oita.jp/?s=${encodeURIComponent(q)}`),
    keep: /\/(?:kurashi|siseijouhou|kankou|article_link|article_[0-9]+)[^"'<> ]*(?:$|\/|\?)/i,
    crawl: /\/(?:kurashi|siseijouhou|kankou|article_link|article_[0-9]+)(?:\/|$|\?)/i,
    limit: 260,
  },
  {
    org: '国東市',
    base: 'https://www.city.kunisaki.oita.jp',
    seeds: ['/', '/sitemap.html', '/life/1/', '/life/2/', '/life/4/', '/life/5/', '/life/6/', '/soshiki/', '/site/kanko/'],
    searches: keywords.map((q) => `https://www.city.kunisaki.oita.jp/site/search/?keyword=${encodeURIComponent(q)}`),
  ...
  {
    org: '姫島村',
    base: 'https://www.himeshima.jp',
    seeds: ['/', '/sitemap.xml', '/sitemap/', '/kurashi/', '/ijuu/', '/news/', '/himeshima-news/', '/about/', '/geopark-news/', '/furusato-ouenkifukin/'],
    searches: keywords.map((q) => `https://www.himeshima.jp/?s=${encodeURIComponent(q)}`),
    keep: /\/(?:kurashi|ijuu|news|himeshima-news|about|geopark-news|furusato-ouenkifukin)[^"'<> ]*(?:$|\/|\?)/i,
    crawl: /\/(?:kurashi|ijuu|news|himeshima-news|about|geopark-news|sitemap)(?:\/|$|\?)/i,
    limit: 900,
  },
];
...
function titleOf(html, fallback) {
  ...
  const primary = /^(宇佐市|豊後大野市|由布市|国東市|姫島村)$/.test(h1Text) ? (og || titleTag || h2 || h1) : (h1 || h2 || og || titleTag);
  return stripHtml(primary || fallback)
    .replace(/\s*[|｜].*$/, '')
    .replace(/\s*[-ー－]\s*(宇佐市|豊後大野市|由布市|国東市|姫島村).*$/, '')
    .trim();
}
