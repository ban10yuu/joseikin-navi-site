...
const { selections } = JSON.parse(fs.readFileSync('tasks/discovery/nagasaki-official-coverage-005-selections.json','utf8'));
const outBase='tasks/discovery/nagasaki-official-coverage-005';
const browserLinksPath = outBase + '-browser-links.json';
const focusedLinksPath = outBase + '-focused-link-scan.json';
const cachedPages = fs.existsSync(browserLinksPath)
  ? JSON.parse(fs.readFileSync(browserLinksPath, 'utf8')).candidates
  : [];
const focusedPages = fs.existsSync(focusedLinksPath)
  ? JSON.parse(fs.readFileSync(focusedLinksPath, 'utf8')).items
  : [];
const cachedByUrl = new Map();
for (const page of [...focusedPages, ...cachedPages]) {
  const title = page.title || '';
  const evidence = page.evidence || '';
  cachedByUrl.set(page.url, {
    status: page.status || 200,
    url: page.url,
    text: `${title} ${evidence}`.replace(/\s+/g, ' ').trim(),
  });
}
...
fs.writeFileSync(outBase+'-ts-snippet.ts',`const nagasakiOfficial005Grants: Array<{
  slug: string;
  title: string;
  organization: string;
  maxAmount: string;
