...
const batch = 'oita-official-coverage-001';
const keywords = ['補助金', '助成金', '給付金', '支援金', '手当', '医療費助成', '就学援助', '奨学金', '貸付', '減免', '移住', '空き家', '耐震', '無償化'];

const orgs = [
  {
    org: '大分市',
    base: 'https://www.city.oita.oita.jp',
    seeds: ['/', '/sitemap.xml', '/sitemap/', '/kurashi/index.html', '/kosodate/index.html', '/kenko/index.html', '/machizukuri/index.html', '/shigotosangyo/index.html'],
    searches: keywords.map((q) => `https://www.city.oita.oita.jp/search.html?q=${encodeURIComponent(q)}`),
    keep: /\/(?:[a-z0-9_-]+\/){0,5}[a-z0-9_-]+\.html(?:$|\?)/i,
    crawl: /\/(?:kurashi|kosodate|kenko|machizukuri|shigotosangyo|bunkasports|shisejoho|sitemap)(?:\/|$)|\.xml$/i,
    limit: 900,
  },
  {
    org: '別府市',
    base: 'https://www.city.beppu.oita.jp',
    seeds: ['/', '/seikatu/', '/seikatu/kenkou_iryou/kyuufu_jyosei/', '/seikatu/kosodate/', '/seikatu/fukusi/', '/seikatu/sumai/', '/sangyou/'],
    searches: keywords.map((q) => `https://www.city.beppu.oita.jp/search.php?keyword=${encodeURIComponent(q)}`),
    keep: /\/(?:seikatu|sangyou|keizai|kankou|kurashi|kosodate|fukushi|iryou|iju|sumai|benri)[^"'<> ]*(?:\/|\.html|\.php)(?:$|\?)/i,
    crawl: /\/(?:seikatu|sangyou|keizai|kankou|kurashi|kosodate|fukushi|iryou|iju|sumai|benri)(?:\/|$)/i,
    limit: 950,
  },
  {
    org: '中津市',
    base: 'https://www.city-nakatsu.jp',
    seeds: ['/', '/sitemap.xml', '/sitemap.html', '/navigation/kurashi/', '/navigation/jigyosha/', '/categories/bunya/fukushi/', '/categories/bunya/kenkoyobo/', '/categories/bunya/sangyokoyo/', '/categories/bunya/jutakuinfura/'],
    searches: keywords.map((q) => `https://www.city-nakatsu.jp/search?keyword=${encodeURIComponent(q)}`),
    keep: /\/(?:doc|categories|navigation|soshiki)\/[^"'<> ]*(?:$|\/|\?)/i,
    crawl: /\/(?:categories|navigation|soshiki|sitemap)(?:\/|\.html|\.xml|$)/i,
    limit: 950,
  },
  {
    org: '日田市',
    base: 'https://www.city.hita.oita.jp',
    seeds: ['/', '/sitemap.html', '/site/kosodate/', '/site/iju/', '/navi/shinsei/', '/life/1/', '/life/2/', '/life/3/', '/life/4/'],
    searches: keywords.map((q) => `https://www.city.hita.oita.jp/site/search/?keyword=${encodeURIComponent(q)}`),
    keep: /\/(?:soshiki|site|life|navi)\/[^"'<> ]*(?:\.html|\/)(?:$|\?)/i,
    crawl: /\/(?:soshiki|site|life|navi|sitemap)(?:\/|\.html|$)/i,
    limit: 950,
  },
  {
    org: '佐伯市',
    base: 'https://www.city.saiki.oita.jp',
    seeds: ['/', '/default.html', '/sitemap.html', '/list00301.html', '/list00302.html', '/list00303.html', '/list00304.html', '/list00305.html', '/list00306.html', '/list00307.html', '/list00308.html', '/list00383.html'],
    searches: keywords.map((q) => `https://www.city.saiki.oita.jp/dynamic/search/pub/default.aspx?c_id=3&q=${encodeURIComponent(q)}`),
    keep: /\/(?:kiji\d+\/index\.html|list\d+\.html|classset\d+\.html|default\.html)(?:$|\?)/i,
    crawl: /\/(?:list\d+\.html|classset\d+\.html|default\.html|sitemap\.html)(?:$|\?)/i,
    limit: 900,
  },
];
...
function titleOf(html, fallback) {
  ...
  return stripHtml(h1 || h2 || titleTag || fallback)
    .replace(/\s*[|｜].*$/, '')
    .replace(/\s*[-ー－]\s*(大分市|別府市|中津市|日田市|佐伯市).*$/, '')
    .trim();
}
