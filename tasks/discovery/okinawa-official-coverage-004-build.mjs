...
const { selections } = JSON.parse(fs.readFileSync('tasks/discovery/okinawa-official-coverage-004-selections.json', 'utf8'));
const outBase = 'tasks/discovery/okinawa-official-coverage-004';
...
for (let i = 0; i < selections.length; i++) {
  ...
  if (page.status !== 200 && fallbackEvidence) {
  ...
  adopted.push({
    slug: `okinawa-official-004-${String(i + 1).padStart(3, '0')}-2026`,
    title: `${org} ${name}`,
    organization: org,
...
fs.writeFileSync(`${outBase}-ts-snippet.ts`, `const okinawaOfficial004Grants: Array<{
  slug: string;
  title: string;
  organization: string;
  maxAmount: string;
