...
const batch = 'kagawa-official-coverage-002';
const outFile = `tasks/discovery/${batch}-rescored-candidates.json`;

const municipalities = [
  { org: '東かがわ市', base: 'https://www.higashikagawa.jp', seeds: ['https://www.higashikagawa.jp/sitemap.xml'], mode: 'sitemap', max: 850 },
  { org: '三豊市', base: 'https://www.city.mitoyo.lg.jp', seeds: ['https://www.city.mitoyo.lg.jp/sitemap.xml'], mode: 'sitemap', max: 850 },
  { org: '土庄町', base: 'https://www.town.tonosho.kagawa.jp', seeds: ['https://www.town.tonosho.kagawa.jp/sitemap.xml'], mode: 'sitemap', max: 850 },
  { org: '小豆島町', base: 'https://www.town.shodoshima.lg.jp', seeds: ['https://www.town.shodoshima.lg.jp/sitemap.xml'], mode: 'sitemap', max: 850 },
];
...
function usefulUrl(orgBase, url) {
  ...
  return pathHint.test(decodeURIComponent(url)) || /\/(soshiki|site|page|docs|kurashi|life)\//.test(url);
}
...
async function collectUrls(m) {
  ...
  while (queue.length && urls.size < m.max) {
    ...
    for (const link of linksFrom(url, page.html)) {
      ...
      if ((/\/(life|soshiki|site)\//.test(link) || pathHint.test(decodeURIComponent(link))) && queue.length < 900) queue.push(link);
    }
  }
  return [...urls].slice(0, m.max);
}
