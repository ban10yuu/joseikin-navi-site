...
const { selections } = JSON.parse(fs.readFileSync('tasks/discovery/kagoshima-official-coverage-001-selections.json', 'utf8'));
const outBase = 'tasks/discovery/kagoshima-official-coverage-001';
...
for (let i = 0; i < selections.length; i++) {
  ...
  adopted.push({
    slug: `kagoshima-official-001-${String(i + 1).padStart(3, '0')}-2026`,
    title: `${org} ${name}`,
    organization: org,
...
fs.writeFileSync(`${outBase}-ts-snippet.ts`, `const kagoshimaOfficial001Grants: Array<{
  slug: string;
  title: string;
  organization: string;
  maxAmount: string;
