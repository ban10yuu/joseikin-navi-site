...
const { selections } = JSON.parse(fs.readFileSync('tasks/discovery/miyazaki-official-coverage-003-selections.json', 'utf8'));
const outBase = 'tasks/discovery/miyazaki-official-coverage-003';
...
for (let i = 0; i < selections.length; i++) {
  ...
  adopted.push({
    slug: `miyazaki-official-003-${String(i + 1).padStart(3, '0')}-2026`,
    title: `${org} ${name}`,
    organization: org,
...
fs.writeFileSync(`${outBase}-ts-snippet.ts`, `const miyazakiOfficial003Grants: Array<{
  slug: string;
  title: string;
  organization: string;
  maxAmount: string;
