...
const { selections } = JSON.parse(fs.readFileSync('tasks/discovery/kumamoto-official-coverage-002-selections.json','utf8'));
const outBase='tasks/discovery/kumamoto-official-coverage-002';
...
function focusText(text,title){const i=text.indexOf(title);return i>=0?text.slice(i,Math.min(text.length,i+3000)):text}
function application(text,title){const body=focusText(text||title,title);const date=body.match(/20\d{2}年\d{1,2}月\d{1,2}日|令和[0-9０-９]+年[0-9０-９]+月[0-9０-９]+日/)?.[0];let phrase=body.match(/(?:申請|募集|受付|締切|期限|交付|支給|請求|提出)[^。．]{0,110}/)?.[0]||body.slice(0,120);phrase=phrase.split(/[｜|/]/)[0].trim();if(/申請書・様式|各種補助金|本文へ|Foreign|フッターメニュー|緊急情報/.test(phrase))phrase='公式ページで申請・受付状況を確認';return (date?date+' ':'')+phrase.slice(0,150)}
async function fetchPage(url){const c=new AbortController();const t=setTimeout(()=>c.abort(),8000);try{const r=await fetch(url,{redirect:'follow',signal:c.signal,headers:{'user-agent':'Mozilla/5.0 official-coverage-check'}});const buf=Buffer.from(await r.arrayBuffer());const contentType=r.headers.get('content-type')||'';const charset=contentType.match(/charset=([^;]+)/i)?.[1]?.toLowerCase();const fallback=/city\.tamana\.lg\.jp|city\.kamiamakusa\.kumamoto\.jp/.test(r.url||url)?'euc-jp':'utf-8';const encoding=charset||fallback;const html=new TextDecoder(encoding).decode(buf);return{status:r.status,url:r.url,text:stripHtml(html)}}catch(e){return{status:0,url,text:String(e?.message||e)}}finally{clearTimeout(t)}}
async function fetchPageWithRetry(url){let last;for(let i=0;i<3;i++){last=await fetchPage(url);if(last.status===200)return last;await new Promise(r=>setTimeout(r,500*(i+1)));}return last}
const adopted=[];const non200=[];for(let i=0;i<selections.length;i++){const [org,name,url]=selections[i];const page=await fetchPageWithRetry(url);if(/\.pdf(?:$|\?)/i.test(url))page.text=`${name} 公式PDF 申請・受付は公式PDF記載に従う`;if(page.status!==200)non200.push({org,name,url,status:page.status,final:page.url});const cat=category(name);const focused=focusText(page.text,name);const amount=maxAmount(name,focused);adopted.push({slug:'kumamoto-official-002-'+String(i+1).padStart(3,'0')+'-2026',title:org+' '+name,organization:org,maxAmount:amount.text,maxAmountNum:amount.num,category:cat,relatedCategories:related(name,cat),eligibility:eligibility(org,name,cat),applicationPeriod:application(focused,name),officialUrl:page.url||url,sourceName:name,status:page.status,evidence:focused.slice(0,220)});}
...
fs.writeFileSync(outBase+'-adopted.tsv',['slug\torganization\ttitle\tmaxAmount\turl\tevidence',...adopted.map(g=>[g.slug,g.organization,g.title,g.maxAmount,g.officialUrl,g.evidence.replace(/\t/g,' ').trim()].join('\t'))].join('\n')+'\n');
const body=JSON.stringify(adopted.map(({status,evidence,...g})=>g),null,2);
fs.writeFileSync(outBase+'-ts-snippet.ts',`const kumamotoOfficial002Grants: Array<{
  slug: string;
  title: string;
  organization: string;
  maxAmount: string;
