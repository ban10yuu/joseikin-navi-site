...
const { selections } = JSON.parse(fs.readFileSync('tasks/discovery/miyazaki-official-coverage-002-selections.json', 'utf8'));
const outBase = 'tasks/discovery/miyazaki-official-coverage-002';
...
for (let i = 0; i < selections.length; i++) {
  ...
  adopted.push({
    slug: `miyazaki-official-002-${String(i + 1).padStart(3, '0')}-2026`,
    title: `${org} ${name}`,
    organization: org,
...
fs.writeFileSync(`${outBase}-ts-snippet.ts`, `const miyazakiOfficial002Grants: Array<{
  slug: string;
  title: string;
  organization: string;
  maxAmount: string;
