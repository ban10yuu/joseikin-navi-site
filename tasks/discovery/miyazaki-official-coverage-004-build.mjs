...
const { selections } = JSON.parse(fs.readFileSync('tasks/discovery/miyazaki-official-coverage-004-selections.json', 'utf8'));
const outBase = 'tasks/discovery/miyazaki-official-coverage-004';
...
async function fetchPage(url) {
  const tsunoArticleId = url.match(/https:\/\/www\.town\.tsuno\.lg\.jp\/article\?articleId=([0-9a-f]+)/)?.[1];
  if (tsunoArticleId) {
    const app = await fetchPage('https://www.town.tsuno.lg.jp/main.106e796a229ee7b4.js');
    const key = app.text.match(/subscriptionKey:"([^"]+)"/)?.[1];
    const response = await fetch('https://www.town.tsuno.lg.jp/prd/tno/portal/openapi/v1/article/detail/retrieve', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ tenantId: '2', siteId: '201', langCode: 'JPN', articleId: tsunoArticleId, pageId: 'PTCGS52' }),
    });
    const json = await response.json();
    const data = json.data?.jpn || {};
    const parts = [data.title, data.summary].filter(Boolean);
    for (const item of data.contentsSec?.textContentSec || []) {
      if (item.textContent) parts.push(item.textContent);
      if (item.linkDisplayName) parts.push(item.linkDisplayName);
      if (item.linkUrl) parts.push(item.linkUrl);
    }
    return { status: response.status, url, text: parts.join(' ').replace(/\s+/g, ' ').trim() };
  }
  try {
...
for (let i = 0; i < selections.length; i++) {
  ...
  adopted.push({
    slug: `miyazaki-official-004-${String(i + 1).padStart(3, '0')}-2026`,
    title: `${org} ${name}`,
    organization: org,
...
fs.writeFileSync(`${outBase}-ts-snippet.ts`, `const miyazakiOfficial004Grants: Array<{
  slug: string;
  title: string;
  organization: string;
  maxAmount: string;
