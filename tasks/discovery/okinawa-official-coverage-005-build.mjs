...
const { selections } = JSON.parse(fs.readFileSync('tasks/discovery/okinawa-official-coverage-005-selections.json', 'utf8'));
const outBase = 'tasks/discovery/okinawa-official-coverage-005';
...
function category(title) {
  ...
  if (/児童|子ども|こども|子育て|母子|父子|ひとり親|養育|保育|給食|新生児|乳幼児|小児|妊婦|産後|助産/.test(title)) return 'childcare';
...
for (let i = 0; i < selections.length; i++) {
  ...
  adopted.push({
    slug: `okinawa-official-005-${String(i + 1).padStart(3, '0')}-2026`,
    title: `${org} ${name}`,
    organization: org,
...
fs.writeFileSync(`${outBase}-ts-snippet.ts`, `const okinawaOfficial005Grants: Array<{
  slug: string;
  title: string;
  organization: string;
  maxAmount: string;
