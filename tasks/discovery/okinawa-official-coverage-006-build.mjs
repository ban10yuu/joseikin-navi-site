...
const { selections } = JSON.parse(fs.readFileSync('tasks/discovery/okinawa-official-coverage-006-selections.json', 'utf8'));
const outBase = 'tasks/discovery/okinawa-official-coverage-006';
...
for (let i = 0; i < selections.length; i++) {
  ...
  adopted.push({
    slug: `okinawa-official-006-${String(i + 1).padStart(3, '0')}-2026`,
    title: `${org} ${name}`,
    organization: org,
...
fs.writeFileSync(`${outBase}-ts-snippet.ts`, `const okinawaOfficial006Grants: Array<{
  slug: string;
  title: string;
  organization: string;
  maxAmount: string;
