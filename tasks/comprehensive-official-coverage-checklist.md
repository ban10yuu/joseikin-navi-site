...
- 公式sitemap/検索/カテゴリBFS候補: 365件（日出町88、九重町117、玖珠町160）。由布市は公式カテゴリ11ページを3秒タイムアウトの軽量方式で確認し、制度強語候補0件。
...
- 低優先/第2巡: RSS、カテゴリ、検索結果、申請書単体、終了済み、外部制度、一般案内、同一制度の重複ページは採用せず、候補JSON/TSVに残置。由布市は003/004とも候補0件だが、未判定解消のため次回の最終補完対象として保持。
...
- メトリクス: 3自治体採用、1自治体保留、60制度採用、一次候補365件、採用率は一次候補比16.4%。大分県001-004累計は18自治体相当310制度（由布市は未完了保留）。
