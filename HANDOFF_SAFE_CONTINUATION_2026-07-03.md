...
## 2026-07-05 追記: このファイルを最初に丸ごと読まない

通常再開時は `CONTEXT_START_HERE.md` を読むこと。必要な場合だけ、このファイルを `tail` または狭い `rg` で参照する。

巨大な永続 goal と長大な引き継ぎファイルの読み込みが原因で、Codex が repeatedly `context window` 上限に到達していた。2026-07-05 に巨大 goal は blocked にし、以後は小バッチ単位で手動継続する。
