...
確認: `npx eslint src/data/grants/verified-local-misc-2026.ts` エラー0、`git diff --check -- src/data/grants/verified-local-misc-2026.ts` 問題なし、採用sourceUrls 10件はすべてHTTP 200。`npm run audit:coverage` は failures 0、公式確認済みactiveは2,637件、群馬県ローカル公式確認済みは56件。`NEXT_PRIVATE_BUILD_WORKER_COUNT=4 NODE_OPTIONS=--max-old-space-size=4096 npm run build` は成功し、静的ページ6,072件生成、`/grant/[slug]` は3,083件相当。

次回再開位置:

- 綾部市残り2件（`ayabe-bicycle-helmet` / `ayabe-tea-business`）は現行公式制度としての確認が弱いため引き続き保留。
- 全国raw gapの次候補は群馬県の高崎市（`takasaki-birth-bonus` / `takasaki-child-medical` / `takasaki-childcare-subsidy` / `takasaki-elderly-support` / `takasaki-infertility` など）。
- コンテキスト溢れ対策として、再開時は `HANDOFF_CONTEXT_RECOVERY_2026-07-03.md` の末尾120行と `node scripts/audit-raw-verified-gaps.mjs --limit 25` のみ読む。`SAFE_CONTINUATION_PLAN.md` と `tasks/todo.md` は全文を読まない。
- push / 公開反映は明示確認後。
...
確認: `npx eslint src/data/grants/verified-local-misc-2026.ts` エラー0、`git diff --check -- src/data/grants/verified-local-misc-2026.ts` 問題なし、採用sourceUrls 14件はすべてHTTP 200。`npm run audit:coverage` は failures 0、公式確認済みactiveは2,629件、群馬県ローカル公式確認済みは48件。`NEXT_PRIVATE_BUILD_WORKER_COUNT=4 NODE_OPTIONS=--max-old-space-size=4096 npm run build` は成功し、静的ページ6,059件生成、`/grant/[slug]` は3,074件相当。

次回再開位置:

- 綾部市残り2件（`ayabe-bicycle-helmet` / `ayabe-tea-business`）は現行公式制度としての確認が弱いため引き続き保留。
- 全国raw gapの次候補は群馬県の桐生市残り（`kiryu-housing-reform` / `kiryu-newlywed-rent` / `kiryu-nursing-equipment` / `kiryu-nursing-home-reform` / `kiryu-scholarship` など）。
- コンテキスト溢れ対策として、再開時は `HANDOFF_CONTEXT_RECOVERY_2026-07-03.md` の末尾120行と `node scripts/audit-raw-verified-gaps.mjs --limit 25` のみ読む。`SAFE_CONTINUATION_PLAN.md` と `tasks/todo.md` は全文を読まない。
- push / 公開反映は明示確認後。
...
## 2026-07-05 熊本県Batch 185 追加ログ

熊本市5件を公式一次情報で確認し、`src/data/grants/verified-local-misc-2026.ts` に追加・補正した。対象は `kumamoto-city-housing-eco` / `kumamoto-city-housing-reconstruction` / `kumamoto-city-school-lunch` / `kumamoto-city-seismic-diagnosis` / `kumamoto-city-senior-medical`。生成データ名をそのまま採用せず、公式ページ・公式PDFで確認できる制度へ補正した。`kumamoto-city-housing-eco` は熊本市省エネルギー機器等導入推進事業補助金、`kumamoto-city-housing-reconstruction` は空き家のリフォーム補助、`kumamoto-city-school-lunch` は令和8年度就学援助、`kumamoto-city-seismic-diagnosis` は戸建木造住宅の耐震改修等事業、`kumamoto-city-senior-medical` は重度心身障がい者（児）医療費助成へ補正した。熊本県raw gapは30件から25件、全国raw gapは2,080件から2,075件に減少した。

確認: `npx eslint src/data/grants/verified-local-misc-2026.ts` エラー0、`git diff --check -- src/data/grants/verified-local-misc-2026.ts` 問題なし、採用sourceUrls 15件はすべてHTTP 200。`npm run audit:coverage` は failures 0、公式確認済みactiveは2,576件、熊本県ローカル公式確認済みは31件。`NEXT_PRIVATE_BUILD_WORKER_COUNT=4 NODE_OPTIONS=--max-old-space-size=4096 npm run build` は成功し、静的ページ5,969件生成、`/grant/[slug]` は3,018件相当。

次回再開位置:

- 綾部市残り2件（`ayabe-bicycle-helmet` / `ayabe-tea-business`）は現行公式制度としての確認が弱いため引き続き保留。
- 全国raw gapの次候補は熊本県（`kumamoto-city-uij-turn` / `kumamoto-elderly-taxi` / `kumamoto-health-checkup-subsidy` など）。
- コンテキスト溢れ対策として、再開時は `HANDOFF_CONTEXT_RECOVERY_2026-07-03.md` と `node scripts/audit-raw-verified-gaps.mjs --limit 25` のみ読む。`SAFE_CONTINUATION_PLAN.md` と `tasks/todo.md` は全文を読まない。
- push / 公開反映は明示確認後。

## 2026-07-05 熊本県Batch 186 追加ログ

熊本市5件を公式一次情報で確認し、`src/data/grants/verified-local-misc-2026.ts` に追加・補正した。対象は `kumamoto-city-uij-turn` / `kumamoto-elderly-taxi` / `kumamoto-health-checkup-subsidy` / `kumamoto-juutaku-reform` / `kumamoto-scholarship-repayment`。生成データ名をそのまま採用せず、公式ページ・公式PDFで確認できる制度へ補正または掲載停止した。`kumamoto-city-uij-turn` は熊本市転居費等支援金、`kumamoto-health-checkup-subsidy` は特定健診・特定保健指導へ補正した。`kumamoto-elderly-taxi` は高齢者のみ対象の公式制度を確認できないため掲載停止、`kumamoto-juutaku-reform` は一般住宅リフォーム助成金として確認できないため掲載停止、`kumamoto-scholarship-repayment` は熊本市独自制度として確認できないため掲載停止した。熊本県raw gapは25件から20件、全国raw gapは2,075件から2,070件に減少した。

確認: `npx eslint src/data/grants/verified-local-misc-2026.ts` エラー0、`git diff --check -- src/data/grants/verified-local-misc-2026.ts` 問題なし、採用sourceUrls 15件はすべてHTTP 200。`npm run audit:coverage` は failures 0、公式確認済みactiveは2,581件、熊本県ローカル公式確認済みは36件。`NEXT_PRIVATE_BUILD_WORKER_COUNT=4 NODE_OPTIONS=--max-old-space-size=4096 npm run build` は成功し、静的ページ5,976件生成、`/grant/[slug]` は3,023件相当。

次回再開位置:

- 綾部市残り2件（`ayabe-bicycle-helmet` / `ayabe-tea-business`）は現行公式制度としての確認が弱いため引き続き保留。
- 全国raw gapの次候補は熊本県（`kumamoto-school-lunch-subsidy` / `kumamoto-startup-support` / `kumamoto-water-saving` など）。
- コンテキスト溢れ対策として、再開時は `HANDOFF_CONTEXT_RECOVERY_2026-07-03.md` と `node scripts/audit-raw-verified-gaps.mjs --limit 25` のみ読む。`SAFE_CONTINUATION_PLAN.md` と `tasks/todo.md` は全文を読まない。
- push / 公開反映は明示確認後。

## 2026-07-05 熊本県Batch 187 追加ログ

熊本市3件・天草市2件を公式一次情報で確認し、`src/data/grants/verified-local-misc-2026.ts` に追加・補正した。対象は `kumamoto-school-lunch-subsidy` / `kumamoto-startup-support` / `kumamoto-water-saving` / `amakusa-block-wall-removal` / `amakusa-child-medical-aid`。`kumamoto-school-lunch-subsidy` は既に公式補正済みの熊本市令和8年度就学援助と重複するため掲載停止、`kumamoto-startup-support` は熊本市商店街出店支援事業、`kumamoto-water-saving` は熊本市雨水貯留施設補助制度、`amakusa-block-wall-removal` は天草市危険ブロック塀等安全確保支援事業、`amakusa-child-medical-aid` は天草市子ども医療費助成制度へ補正した。熊本県raw gapは20件から15件、全国raw gapは2,070件から2,065件に減少した。

確認: `npx eslint src/data/grants/verified-local-misc-2026.ts` エラー0、`git diff --check -- src/data/grants/verified-local-misc-2026.ts` 問題なし、採用sourceUrls 10件はすべてHTTP 200。`npm run audit:coverage` は failures 0、公式確認済みactiveは2,586件、熊本県ローカル公式確認済みは41件。初回 `npm run build` は `relatedCategories` に存在しない `business` を指定してTypeScriptで停止したため `living` に修正。再実行した `NEXT_PRIVATE_BUILD_WORKER_COUNT=4 NODE_OPTIONS=--max-old-space-size=4096 npm run build` は成功し、静的ページ5,982件生成、`/grant/[slug]` は3,028件相当。
...
## 2026-07-05 熊本県Batch 189 追加ログ

八代市5件を公式一次情報で確認し、`src/data/grants/verified-local-misc-2026.ts` に追加・補正した。対象は `yatsushiro-block-wall-removal` / `yatsushiro-child-medical-aid` / `yatsushiro-elderly-taxi` / `yatsushiro-health-checkup-subsidy` / `yatsushiro-juutaku-reform`。`yatsushiro-block-wall-removal` は八代市危険ブロック塀等除却促進事業、`yatsushiro-child-medical-aid` は八代市こども医療費助成制度、`yatsushiro-elderly-taxi` は八代市高齢者外出支援事業、`yatsushiro-health-checkup-subsidy` は人間ドックの情報提供報奨金事業、`yatsushiro-juutaku-reform` は八代市空き家バンク活用促進事業補助金へ補正した。熊本県raw gapは10件から5件、全国raw gapは2,060件から2,055件に減少した。

確認: `npx eslint src/data/grants/verified-local-misc-2026.ts` エラー0、`git diff --check -- src/data/grants/verified-local-misc-2026.ts` 問題なし、採用sourceUrls 12件はすべてHTTP 200。`npm run audit:coverage` は failures 0、公式確認済みactiveは2,595件、熊本県ローカル公式確認済みは50件。`NEXT_PRIVATE_BUILD_WORKER_COUNT=4 NODE_OPTIONS=--max-old-space-size=4096 npm run build` は成功し、静的ページ5,999件生成、`/grant/[slug]` は3,038件相当。
...
## 2026-07-05 群馬県Batch 200 追加ログ

前橋市5件を公式一次情報で確認し、`src/data/grants/verified-local-misc-2026.ts` に追加・補正した。対象は `maebashi-afterschool` / `maebashi-birth-bonus` / `maebashi-disability-medical` / `maebashi-elderly-support` / `maebashi-housing-acquisition`。`maebashi-afterschool` は市全体の利用料減免制度として確認できないため掲載停止、`maebashi-birth-bonus` は妊婦のための支援給付、`maebashi-disability-medical` は福祉医療制度の重度心身障害者等、`maebashi-elderly-support` は緊急通報システム事業、`maebashi-housing-acquisition` は令和8年度移住支援金へ補正した。群馬県raw gapは20件から15件、全国raw gapは2,001件から1,996件に減少した。

確認: `npx eslint src/data/grants/verified-local-misc-2026.ts` エラー0、`git diff --check -- src/data/grants/verified-local-misc-2026.ts` 問題なし、採用sourceUrls 9件はすべてHTTP 200。`npm run audit:coverage` は failures 0、公式確認済みactiveは2,650件、群馬県ローカル公式確認済みは69件。`NEXT_PRIVATE_BUILD_WORKER_COUNT=4 NODE_OPTIONS=--max-old-space-size=4096 npm run build` は成功し、静的ページ6,092件生成、`/grant/[slug]` は3,098件相当。
...
## 2026-07-05 広島県Batch 204 追加ログ

呉市残り8件を公式一次情報で確認し、`src/data/grants/verified-local-misc-2026.ts` に追加・補正した。対象は `kure-block-wall-removal` / `kure-child-medical-aid` / `kure-elderly-taxi` / `kure-health-checkup-subsidy` / `kure-juutaku-reform` / `kure-scholarship-repayment` / `kure-school-lunch-subsidy` / `kure-water-saving`。ブロック塀等安全確保事業、こども医療費助成制度、生活バス路線等優待制度、特定健診・がん検診、新婚・子育て世帯定住支援事業、呉市職員奨学金返還支援制度、学校給食費の実質無償化、省エネ家電・省エネ設備導入助成へ補正した。呉市raw gapは0件になり、全国raw gapは1,972件から1,964件、広島県raw gapは74件から66件に減少した。

確認: `npx eslint src/data/grants/verified-local-misc-2026.ts` エラー0、`git diff --check -- src/data/grants/verified-local-misc-2026.ts` 問題なし、採用sourceUrls 15件はすべてHTTP 200。`npm run audit:coverage` は failures 0、公式確認済みactiveは2,680件、広島県ローカル公式確認済みは39件。Buildは今回は未実行。

次回再開位置:

- 綾部市残り2件（`ayabe-bicycle-helmet` / `ayabe-tea-business`）は現行公式制度としての確認が弱いため引き続き保留。
- 次は広島県庁 `hiroshima-education-ict` / `hiroshima-migration-family` または広島市 `hiroshima-block-wall-removal` / `hiroshima-child-medical-aid` など。
- コンテキスト溢れ対策として、再開時は `HANDOFF_CONTEXT_RECOVERY_2026-07-03.md` の末尾120行と `node scripts/audit-raw-verified-gaps.mjs --limit 25` のみ読む。`SAFE_CONTINUATION_PLAN.md` と `tasks/todo.md` は全文を読まない。
- push / 公開反映は明示確認後。
