...
- トップの主要導線だけ人物入りフラットイラストを使い、制度カードや公式CTAには装飾画像を追加しない。
- カテゴリと目的は、同一シートから切り出したモチーフアイコンで統一する。
- 輪郭は濃紺 `#17345f`、面は青 `#2557a7`、緑 `#25846b`、琥珀 `#d58a2a`、珊瑚 `#df806f` の範囲で使う。
- 画像内文字、ロゴ、行政紋章、通貨記号、グラデーション、光沢、3D、写実、強い影を使わない。
- 人物の顔と手は簡略化し、細かい指や誇張した表情を描かない。
- 装飾画像は空の `alt`、意味を補足する画像だけ短い日本語の `alt` を付ける。
- 画像が取得できなくても、見出し、説明、操作だけで内容が成立するようにする。
